...
async function runRealtimeE2ETest() {
  ...
  await Promise.all([
    new Promise((resolve) => pmSocket.on('connect', resolve)),
    new Promise((resolve) => devSocket.on('connect', resolve)),
  ]);
  ...
  const targetTask = tasksList.find((t) => t.status === 'TODO') || tasksList[0];
  ...
  const receivedOnDevPromise = new Promise((resolve, reject) => {
